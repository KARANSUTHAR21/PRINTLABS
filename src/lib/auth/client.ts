...
/**
 * Store a session bearer token (no-op when one is already held). Email
 * sign-in/sign-up call this with the token Better Auth returns: when the
 * environment cannot persist the session cookie (e.g. a dev proxy strips
 * Set-Cookie), the bearer keeps the session alive — the server accepts it via
 * the `bearer` plugin and every server function forwards it.
 */
export function storeSessionToken(token: string | null | undefined): void {
  if (!token || getBearerToken()) return;
