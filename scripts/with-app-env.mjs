...
/**
 * Build the spawn arguments for `shell: true`. On Windows cmd.exe re-parses
 * the whole line, so an unquoted token containing a space — a command like
 * `C:\Program Files\nodejs\node.exe`, or an `-e` snippet — executes as the
 * wrong path. Quote every token there; POSIX keeps plain word-splitting so
 * shell metacharacters in wrapped commands keep working.
 */
export function toShellInvocation(command, args, platform = process.platform) {
...
function main(argv) {
  ...
  // `.env` first so its values flow into the child via `process.env`; an
  // already-set real env var always wins (see load-env.mjs precedence).
  loadEnvFile();
  const env = mergeAppEnv(readAppEnv(projectRoot()), process.env);
  const options = { stdio: "inherit", env, shell: true };
  const child = spawn(...toShellInvocation(command, args), options);
