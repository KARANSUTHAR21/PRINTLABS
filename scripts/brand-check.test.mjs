...
test("SKILL.md and AGENTS.md name the marker path and bound this script uses", () => {
  ...
  for (const rel of [".grok/skills/og/SKILL.md", "AGENTS.md"]) {
    ...
    // This workspace replaced the template AGENTS.md with its own brief, so
    // the marker contract lives only in the skill; the AGENTS.md assertions
    // re-arm on their own if the template section ever returns.
    if (rel === "AGENTS.md" && !doc.includes("og-pending")) continue;
...
test("the sections that own the brand-task prohibition never affirm a wait", () => {
  ...
  // The workspace brief (AGENTS.md) dropped the template's execution-loop
  // section; the skill still owns the prohibition. Sections re-arm when the
  // heading returns.
  const sections = PROHIBITION_SECTIONS.filter((section) =>
    readDoc(section.rel).includes(section.from),
  );
  assert.ok(
    sections.some((section) => section.rel === ".grok/skills/og/SKILL.md"),
    "the skill's brand-task prohibition went missing",
  );
