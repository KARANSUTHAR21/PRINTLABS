...
// Identity expectations below assume the injector sees NO workspace identity
// files: `injectGrokPwaHead` defaults cwd to process.cwd(), and this repo's
// site.json / public/ card carry PrintHub's baked identity, which would leak
// into every default-cwd call. Run the whole file from a clean temp dir.
process.chdir(mkdtempSync(join(tmpdir(), "grok-pwa-identity-cwd-")));
