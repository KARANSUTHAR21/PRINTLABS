...
test("the auth schema ships outside the globbed directory (auth-off workspaces)", () => {
  const migrationsDir = join(projectRoot(), "migrations");
  if (authSchemaCopy(projectRoot()) !== null) {
    // Auth-on workspace (PrintHub): the copy was promoted into the globbed
    // directory by design; byte-identity with its source is the next test.
